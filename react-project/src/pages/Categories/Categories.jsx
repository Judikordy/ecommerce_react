    ...
    <div className="max-w-screen-xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 px-4">
        {categories.map((categ) => (
          <div key={categ.id} className="group bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-200">
            <div className="relative bg-white rounded-xl shadow overflow-hidden group-hover:scale-[1.03] duration-500">
              ...
              <img onClick={() => navigate(`/subcategories/${categ.id}`)} src={categ.image} alt={categ.name} className="w-full h-50 object-cover rounded-lg cursor-pointer"/>
